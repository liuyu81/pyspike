...
size_t
py_commit_log_reg_t::len() const {
    return this->ref.size();
}


bool
py_commit_log_reg_t::contains(reg_t regnum) const {
    return this->ref.contains(regnum);
}


freg_t
py_commit_log_reg_t::getitem(reg_t regnum) const {
    return this->ref[regnum];
}


void
py_commit_log_reg_t::setitem(reg_t regnum, freg_t data) {
    this->ref[regnum] = data;
}


void
py_commit_log_reg_t::setitem(reg_t regnum, py::tuple data) {
    freg_t packed_data = {py::cast<uint64_t>(data[0]), py::cast<uint64_t>(data[1])};
    this->ref[regnum] = packed_data;
}


void
py_commit_log_reg_t::delitem(reg_t regnum) {
    this->ref.erase(regnum);
}


void
py_commit_log_reg_t::clear() {
    this->ref.clear();
}


py::str
py_commit_log_reg_t::repr() const {
...
size_t
py_commit_log_mem_t::len() const {
    return this->ref.size();
}


void
py_commit_log_mem_t::append(const std::tuple<reg_t, uint64_t, uint8_t>& item) {
    this->ref.push_back(item);
}


std::tuple<reg_t, uint64_t, uint8_t>
py_commit_log_mem_t::getitem(int index) {
    return this->ref[index];
}


void
py_commit_log_mem_t::clear() {
    this->ref.clear();
}


py::str
py_commit_log_mem_t::repr() const {
  std::ostringstream oss;
  oss << "{";
  oss << "}";
  return oss.str();
