...
def _patch_version(source_dir: pathlib.Path):
    with source_dir.joinpath("VERSION").open("r+") as f_ver:
        default_version = f_ver.read()
        try:
            semantic_version = getattr(package, "__version__", None)
            if semantic_version is None:
                semantic_version = get_version(local_scheme="setup:local_scheme")
            f_ver.seek(0)
            f_ver.write(f"#define SPIKE_VERSION \"{semantic_version}\"\n")
            f_ver.truncate()
            yield semantic_version
        finally:
            f_ver.seek(0)
...
def local_scheme(version: ScmVersion):
    if version.exact or not version.distance:
        return ""
    spike_commit = subprocess.run([
        "git", "rev-parse", "--short", "HEAD:vendor/spike"
    ], check=True, capture_output=True, text=True).stdout.strip()
    pyspike_commit = subprocess.run([
        "git", "rev-parse", "--short", "HEAD"
    ], check=True, capture_output=True, text=True).stdout.strip()
    return f"+g{pyspike_commit}.spike.g{spike_commit}"
